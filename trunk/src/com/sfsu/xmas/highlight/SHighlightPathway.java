 ...
 * @author bdalziel
 */
public class SHighlightPathway extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            int pathwayID = Integer.parseInt((String) request.getParameter("pathway_id"));
            ...
            if (hm.getHighlightedPathways(sessionID).contains(pathwayID)) {
                hm.stopHighlightingPathway(sessionID, pathwayID);
                out.append("0");
            } else {
                KnowledgeDataSet kDB = SessionAttributeManager.getActiveKnowledgeLibrary(request);
                if (kDB != null) {
                    hm.highlightPathway(sessionID, kDB.getPathway(pathwayID));
                }
                out.append("1");
            }
        } finally {
