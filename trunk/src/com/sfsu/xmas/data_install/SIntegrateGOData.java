...
public class SIntegrateGOData extends HttpServlet {
    ...
    */
    ...
    throws ServletException, IOException {
        ...
        String fileName = request.getParameter("go_file_name");

        GOFileReader fr = new GOFileReader(fileName);
     
        KnowledgeDataSetDAO kDBDAO = DAOFactoryFactory.getUniqueInstance().getDatabaseDAOFactory().getKnowledgeDataSetDAO(kDB.getID());

        boolean tablePopulationSuccessful = kDBDAO.populateGoTermsTable(fr);
        if (tablePopulationSuccessful) {
            System.out.println("");
        }
        kDBDAO.populateGoTermAssignmentTable(fr);
